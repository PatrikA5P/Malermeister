...
import { db } from '../../../db';
import { OfficeDocument, Customer, Project, Product, OfficeAddress, OfficeLineItem } from '../../../officeTypes';
...
const QuotesOverview: React.FC<OffersProps> = ({ onBack, preselectedCustomerId }) => {
  ...
  // Helper to calculate actual gross total excluding optionals
  const calculateDisplayTotal = (doc: OfficeDocument) => {
    if (!doc.items) return 0;
    
    // Default VAT rate if not set on item
    const defaultVat = 8.1; 

    return doc.items.reduce((sum, item: OfficeLineItem) => {
      if (item.isOptional) return sum;
      
      const quantity = Number(item.quantity || 0);
      const price = Number(item.price || 0);
      const discount = Number(item.discount || 0);
      const vatRate = Number(item.vatRate ?? defaultVat);
  ...
  const totalGross = paginatedDocs.reduce((sum, d) => sum + calculateDisplayTotal(d), 0);
  ...
  const exportOffers = async (format: 'csv' | 'xlsx') => {
    try {
      const rows = filteredDocs.map(d => ({
        ...
        Brutto: Number(calculateDisplayTotal(d) ?? 0).toFixed(2),
        Währung: 'CHF',
              ...
              >
                <div className="relative p-4 rounded-xl border border-zinc-200 bg-white shadow-sm transition-all hover:border-olive-400 active:scale-[0.98]">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    <p className="font-bold text-base whitespace-nowrap text-zinc-900">{formatMoney(calculateDisplayTotal(d))}</p>
                  </div>
              ...
              <tbody className="divide-y divide-zinc-100 text-sm text-zinc-700 font-normal">
                {paginatedDocs.map(d => (
                  <tr key={d.id} className={`group hover:bg-olive-50/20 transition-colors ${selectedIds.has(d.id!) ? 'bg-olive-50/40' : ''}`}>
                    ...
                    <td className="p-4 text-right tabular-nums">{formatMoney(calculateDisplayTotal(d)).replace('CHF ', '')}</td>
