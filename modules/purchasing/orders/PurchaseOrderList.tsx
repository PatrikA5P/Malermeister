...
const PurchaseOrderList: React.FC = () => {
    ...
    const load = async () => {
        const d = await db.documents.where('type').equals('purchase_order').reverse().toArray();
        setDocs(d);
    };
    ...
    const handleSave = async (doc: OfficeDocument) => {
        ...
        setEditing(null);
        load();
        setToast({ msg: 'Bestellung gespeichert', type: 'success' });
    };

    if (editing) {
        return <PurchaseEditor initialDoc={editing} onSave={handleSave} onCancel={() => setEditing(null)} title="Bestellung" />;
    }
        ...
        <div className="flex flex-col h-full bg-slate-50 relative">
            ...
            <div className="px-4 md:px-8 pt-4 pb-4">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button onClick={createNew} className="bg-zinc-900 text-white px-6 py-3 rounded-xl text-xs font-black uppercase shadow-lg hover:bg-olive-600 transition-all">+ Bestellung</button>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 md:px-8 pb-20 space-y-3">
                        ...
                        <div className="flex justify-between items-start">
                            <div>
                                <div className="flex items-center gap-2 mb-2">
                                    ...
                                    <span className={`text-[9px] px-2 py-0.5 rounded font-black uppercase ${d.status === 'sent' ? 'bg-blue-50 text-blue-600' : 'bg-zinc-100 text-zinc-500'}`}>{d.status}</span>
                                </div>
                                <h4 className="font-bold text-zinc-900">{d.client.name || 'Neuer Lieferant'}</h4>
                                <p className="text-xs text-zinc-400">{formatDate(d.date)} • {d.items.length} Positionen</p>
                            </div>
                ...
                {docs.length === 0 && <div className="text-center text-zinc-400 py-10 font-bold uppercase text-xs tracking-widest">Keine Bestellungen</div>}
            </div>
        </div>
    );
};
