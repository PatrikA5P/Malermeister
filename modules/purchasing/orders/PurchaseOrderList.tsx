...
const PurchaseOrderList: React.FC<PurchaseOrderListProps> = ({ onBack, onNavigate }) => {
    ...
    if (editing) {
        return <PurchaseEditor initialDoc={editing} onSave={handleSave} onCancel={() => setEditing(null)} title={t('purchasing.order')} />;
    }
        ...
        },
        {
            ...
            render: (row) => (
                <Badge
                    ...
                    variant={row.status === 'sent' ? 'info' : row.status === 'draft' ? 'default' : 'success'}
                />
            )
        },
        {
            ...
            cardPosition: 'meta',
            render: (row) => `${row.items.length} ${t('common.positions')}`
        },
        {
            key: 'totalGross',
                ...
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="flex-1">
                        <div className="flex items-center gap-2">
                            <h2 className="text-lg font-black brand-font uppercase">{t('purchasing.orders')}</h2>
                            {onNavigate && (
                                <select
